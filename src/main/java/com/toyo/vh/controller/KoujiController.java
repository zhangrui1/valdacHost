...
public class KoujiController {
    ...
    public String addKoujiByForm(@ModelAttribute("KoujiForm")KoujiForm koujiForm, ModelMap modelMap,HttpSession session) throws IOException {
        koujiForm.setStatus("企画中");
        //DB更新
...
//        List<Valve> valveList = itemService.getItemByLocationName(kouji.getLocation());
        ...
        if(koujis.size()>0){
            if(kouji.getLocation().equals(koujis.get(0).getLocation())){
                koujis.add(0,kouji);
            }
        }
        session.setAttribute("locationKoujiSelectedForKouji",koujis);
        ...
        luceneIndexService.insertRecord(indexKoujiFile, "KJ" + kouji.getId() + "End", kouji.toText());

        return "redirect:/kouji/"+kouji.getId()+"/valve";
    }
...
//        }
     ...
     *
     * */
    @RequestMapping(value = "/{id}/valve", method = RequestMethod.GET)
    public String showValveList(@PathVariable("id")String id,
        ...
        List<Valve> valveListByL = itemService.getItemByLocationName(kouji.getLocation());
...
//        }
        ...
        session.setAttribute("valveListByL",valveListByL);
        modelMap.addAttribute("valveListByL",valveListByL);
        modelMap.addAttribute("valveList",valveList);
...
//        valveList=itemService.getKikisysByKoujiId(id);
     ...
     *
     * */
    @RequestMapping(value = "/{id}/kiki", method = RequestMethod.GET)
    public String showKikiLst(@PathVariable("id")String id,
                               ModelMap modelMap,HttpSession session) throws IOException {
        Kouji kouji=(Kouji)session.getAttribute("kouji");
        List<Valve> valveList=(List<Valve>)session.getAttribute("valveList");
...
//            }
     ...
     *
     * */
    @RequestMapping(value = "/{id}/kiki", method = RequestMethod.POST)
    public String addKikiList(@PathVariable("id")String id,
                               ModelMap modelMap,HttpSession session) throws IOException {
        Kouji kouji=(Kouji)session.getAttribute("kouji");
        List<Valve> valveList=(List<Valve>)session.getAttribute("valveList");
...
//            Gson gson=new Gson();
//            if(searchResultObjectsKR.size()>0){
////                System.out.println("size="+searchResultObjectsKR.size());
////                System.out.println("kR="+gson.toJson(searchResultObjectsKR));
//                for(int i=0;i<searchResultObjectsKR.size();i++){
//                    Koujirelation TmpKoujirelation=new Koujirelation();
//                    Koujirelation koujirelation=TmpKoujirelation.toKoujirelation(searchResultObjectsKR.get(i).getBody());
//
